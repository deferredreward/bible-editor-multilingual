...
//
// Run from api/:
//   node --experimental-strip-types --no-warnings src/trashedRowPatch.test.mjs
//
...
function db() {
  ...
  d.exec(`CREATE TABLE tn_rows (
    ...
    review_kind TEXT, review_reason TEXT, translation_state TEXT,
...
// real contentPatchClearClauses fragment (plus this fork's translation_state
// demotion CASE, which rows.ts appends for tn). fields = ["note"].
...
// ── tq/twl get NO clear clauses: in this fork only tn_rows carries the
//    review_kind/review_reason/trashed_at columns (migrations 0026/0031), so a
//    clause for the other kinds would 500 every tq/twl content PATCH ──
{
  assert(contentPatchClearClauses("tq").length === 0, "tq gets no clear clauses (columns don't exist on tq_rows)");
  assert(contentPatchClearClauses("twl").length === 0, "twl gets no clear clauses (columns don't exist on twl_rows)");
}
