...
// `push: branches: ['*-be-*']` and the merge workflow re-checks for `-be-` —
// both requiring the *trailing* dash. A suffix-less `{BOOK}-be` matches neither,
// so those branches were never validated and never auto-merged, while Gitea
// still reported a green combined status (no/skipped checks count as success).
// Keep every branch carrying `-be-`.
//
