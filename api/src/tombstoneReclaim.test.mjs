...
// reimportJourney.test.mjs's rationale exactly: a test that hand-copies the
...
//
// Run from api/:
//   node --experimental-strip-types --no-warnings src/tombstoneReclaim.test.mjs
//
// What this covers:
...
//       exactly the pre-fix behavior — not reclaimed, not counted blocked.
...
//   (g) a THROWN reclaim write batch taints apply_incomplete — without this,
//       a failed reclaim write is certified in-sync while master's row is
//       still absent from D1, and the export never retries.
//   (h) reclaim resets restored_from_version to the fresh-insert default on a
//       NON-tn kind (in this fork review_kind/review_reason exist on tn only
//       — migration 0031 — so the shared reset column for tq/twl is
//       restored_from_version; tn's review columns are covered in (f2)).
//   (f2) a tn reclaim also resets review_kind/review_reason alongside
...
console.log("\n[(f2) a tn reclaim resets review_kind/review_reason/restored_from_version to fresh-insert defaults]");
{
  const { sqlite, env } = freshEnv();
  // A tn tombstone carrying the review-metadata columns a human (or the
  // adapt/migrate pipeline) could have set on the OLD row before it was
  // deleted: a "switch to v3" restore chip, and a flag-for-review marker with
  // its reason. None of these describe master's incoming (unrelated) row and
  // must not survive onto it. In this fork review_kind/review_reason exist on
  // tn_rows only (migration 0031).
  sqlite
    .prepare(
      `INSERT INTO tn_rows
         (id, book, chapter, verse, ref_raw, note, sort_order, deleted_at,
          restored_from_version, review_kind, review_reason)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .run(ID, BOOK, 5, 4, "5:4", "old note", 10, 1753900000, 3, "quote_mismatch", "quote could not be re-anchored");
  ...
  const stored = sqlite
    .prepare(`SELECT restored_from_version, review_kind, review_reason FROM tn_rows WHERE book = ? AND id = ?`)
    .all(BOOK, ID);
...
console.log("\n[(h) reclaim resets restored_from_version on a NON-tn kind]");
{
  const { sqlite, env } = freshEnv();
  // A tq tombstone carrying restored_from_version — the one review-metadata
  // column shared by all three TSV kinds in this fork (migration 0007). The
  // OLD row's "showing as v3" chip must not survive onto master's unrelated
  // new content.
  sqlite
    .prepare(
      `INSERT INTO tq_rows
    ...
    .run(ID, BOOK, 5, 4, "5:4", "old question", "old response", 10, 1753900000, null, 3);

  const counts = await applyTsvRows(env, BOOK, "tq", [tqMasterRow()], null);
  eq(counts.tombstone_reclaimed, 1, "reclaimed");

  const stored = sqlite
    .prepare(`SELECT restored_from_version FROM tq_rows WHERE book = ? AND id = ?`)
    .all(BOOK, ID);
  eq(stored[0].restored_from_version, null, "restored_from_version reset — the OLD row's 'showing as v3' chip does not apply to master's new content");
}
...
console.log("\n[(g) a THROWN reclaim batch taints apply_incomplete — never certified in-sync over a still-missing row]");
{
  ...
  eq(counts.apply_incomplete, true, "apply_incomplete IS tainted — a thrown reclaim batch must not be certified in sync");
  ...
  // at the actual call site (`!shouldRecordResourceSync(...) || applyIncomplete`
  // in bookReimport.ts's reimport-sync step), not folded into it — so the gate
  // this asserts is the real combined condition, matching that call site, not
  // shouldRecordResourceSync alone.
  eq(
...
console.log("\n[(i) the write and its boundary log travel in ONE atomic batch() call]");
{
  // The reclaim write and its edit_log 'create' row (the history BOUNDARY
  // rowHistoryBoundary.ts relies on) must land via ONE env.DB.batch() call —
  // never a write batch, then a follow-up JS-meta.changes-gated log batch. A
  // landed write whose follow-up log batch failed independently would leave
  // the boundary permanently missing, since a reclaimed row is no longer a
  // tombstone and a retry can't hit this branch again. Prove the fix
  // directly: count batch() calls (not just outcomes) — there must be exactly
  // ONE per chunk, containing BOTH statements for every reclaimed row.
  const { sqlite, env } = freshEnv();
