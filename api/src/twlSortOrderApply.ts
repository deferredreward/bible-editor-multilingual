...
// post-pass (bookReimport). Sets sort_order + updated_at only — it does NOT bump
// version, does NOT touch content/updated_by, and does NOT write edit_log (a
// reorder is transient last-write-wins, not a versioned content edit). Mirrors
// rows.ts's in-app reorder fast path exactly (same operation, same
...
// content). Idempotent (same updates → same result) and best-effort: a failure
// is logged and swallowed so a sort-order update can never fail the caller.
...
export async function applyTwlSortOrderUpdates(
  ...
  // One UPDATE per row (4 bound values each), chunked into batches. Scoped to
  ...
  // another book. No version bump (issue #687: a reorder is not a versioned
  // content edit, and this must match rows.ts's in-app reorder fast path, which
  // also sets sort_order + updated_at only and never bumps version here).
  const now = Math.floor(Date.now() / 1000);
  const stmt = db.prepare(
    `UPDATE twl_rows SET sort_order = ?1, updated_at = ?2 WHERE book = ?3 AND id = ?4`,
  );
  try {
    for (let i = 0; i < updates.length; i += D1_MAX_STATEMENTS) {
      const slice = updates.slice(i, i + D1_MAX_STATEMENTS);
      await db.batch(slice.map((u) => stmt.bind(u.sort_order, now, book, u.id)));
    }
  } catch (e) {
