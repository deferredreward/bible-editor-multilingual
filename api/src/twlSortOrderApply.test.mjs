// Regression test for issue #687 (ported from unfoldingWord/bible-editor
// ebc2b76): a pure TWL sort-order reorder must not bump `version` (there would
// be nothing in edit_log to explain the bump — the invariant is version
// increments iff content actually changed, and every increment has an edit_log
// row). applyTwlSortOrderUpdates is called from BOTH the nightly export and the
// reimport canonical post-pass to write computed sort_order — this must match
// rows.ts's in-app reorder fast path, which sets sort_order + updated_at only.
// Before this fix it also did `version = version + 1`, so the two paths
// disagreed and a twl row's version could advance with no edit_log row
// explaining it (and could hand open editors spurious If-Match 409s).
//
// (This fork has no rowProvenance subsystem, so the signature is 3 args and
// there are no last_change_* columns — that is the only deliberate divergence
// from the upstream test.)
//
// Run from api/:
...
// Minimal D1 shim over node:sqlite. applyTwlSortOrderUpdates only needs
// .prepare().bind() and .batch().
...
function readRow(sqlite, book, id) {
  return sqlite
    .prepare(`SELECT sort_order, version, updated_at FROM twl_rows WHERE book = ? AND id = ?`)
    .all(book, id)[0];
}
