...
// --- every export branch carries `-be-` so the DCS gates don't skip it ---
{
  ...
  // The one consumer that parses a branch name back (scripts/classify-be-pr.mjs)
  // splits on "-be" and takes [0], so the book still resolves with the suffix.
  assert(buildExportBranch("LAM", []).split("-be")[0] === "LAM", `book still recoverable from a mechanical branch`);
}
