// Regression coverage for the applyVerseRows pristine-write batch: it must
// chunk under D1's 100-statement batch cap (mirroring every other write site
// in bookReimport.ts, which all chunk at WRITE_BATCH = 90), and must count
// inserted/updated only for statements that actually changed a row — not
// blindly, the way the old unchunked implementation did. A chapter with >50
// changed/new verses (PSA 119 after a chapter-wide master change is the
// canonical example) used to throw the single oversized batch() call and
// silently degrade to the per-row fallback for the WHOLE chapter, burning
// exactly the subrequest budget the batching exists to protect.
//
// Run from api/:
//   node --experimental-strip-types --no-warnings src/applyVerseRows.test.mjs
...
// Fork note: applyVerseRows verifies the scripture lane's write identity
// before any write (verifyVerseWriteIdentity → requireLaneState). With the
// full migration set applied and no project_config row, requireLaneState
// lazily seeds default lane rows (ensureLaneState) off the default preset, so
// the ULT lane comes up active at generation 1 with no replacement pending —
// exactly the healthy state the write guards require. No extra seeding needed.
function freshEnv() {
...
{
  // 176 brand-new verses (PSA 119's real verse count) — unchunked, this is
  // 2 statements/verse = 352 statements in one batch() call, well over D1's
  // 100-statement cap. At PRISTINE_PAIR_BATCH = 45 verses per slice (90
  // statements each) that's ceil(176/45) = 4 batch() calls.
  const { env, sqlite } = freshEnv();
  const verses = Array.from({ length: 176 }, (_, i) => verse(119, i + 1, `verse ${i + 1} text`));

  const counts = await applyVerseRowsForTest(env, BOOK, VERSION, verses, null);
  ...
  eq(
    ...
    true,
    "every batch() call stayed at or under the paired-statement cap (≤45 verses × 2 statements)",
  );
  eq(env.DB._batchCalls.count, 4, "writes were split into 4 chunked batch() calls, not one giant 352-statement batch");
}

console.log("\n[a lost UPDATE race (updated_by set between read and write) is not miscounted]");
{
  // A pristine verse is read as editable (updated_by IS NULL), but by the time
  // the UPDATE runs a human has claimed it (updated_by now set) — the UPDATE's
  // own guard matches 0 rows. The old code folded `updated` unconditionally
  // once the batch() call succeeded, over-counting a race it never actually
  // won.
  const { env, sqlite } = freshEnv();
  sqlite
  ...
  sqlite
    .prepare(
      `INSERT INTO verses (book, chapter, verse, verse_end, bible_version, source_generation, content_json, plain_text, version, updated_by)
  ...
  const counts = await applyVerseRowsForTest(env, BOOK, VERSION, [verse(23, 1, "new master text")], null);
...
console.log("\n[a plain no-op (nothing changed) is still counted skipped_noop, not inserted/updated]");
{
  ...
  await applyVerseRowsForTest(env, BOOK, VERSION, [v], null);
  const counts = await applyVerseRowsForTest(env, BOOK, VERSION, [v], null);
...
{
  // A split (write batch, then a separate log batch) would mean the log batch
  // could fail after the write batch had already landed; the fallback would
  // then see the content already matching and count a silent no-op,
  // permanently losing the audit row for a verse whose version really did
  // bump. Proving "one batch() call per chunk" is proving that failure mode
  // is structurally impossible — a thrown batch() can only mean NEITHER the
  // write NOR its log landed (D1 batches are transactional), never one
  // without the other.
  const { env, sqlite } = freshEnv();
...
console.log("\n[a chunk that fails outright falls back to per-row, which still writes both content AND its audit row]");
{
  ...
  // recover through applyVerseRowsPerRow — proving the fallback is now
  // per-slice, not whole-chapter.
