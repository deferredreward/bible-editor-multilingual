...
export function analyzeAlignmentDelta(beforeContent: unknown, afterContent: unknown): AlignmentDelta {
  ...
  // Reporting all of them is what makes guardBlocksSave fire and what lets the
  // export backstop (`usfmAlignmentShrinkRefused`, filtering `reason === "lost"`)
  // refuse a render that flattens a verse. The known cost is case (b): rewording
  // every aligned word in a verse is now refused. `text_edit` gets the "Save
  // anyway" confirm; `find_replace` has no override and skips that verse.
  // The duplicate-surface ambiguity skip further down is bypassed here too —
  // safe direction (it can only add losses), but the reported list may name
  // repeated surfaces the LCS path would decline to attribute.
