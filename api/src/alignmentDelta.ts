...
export function analyzeAlignmentDelta(beforeContent: unknown, afterContent: unknown): AlignmentDelta {
  ...
  // and (b) the export backstop `usfmAlignmentShrinkRefused`, which filters on
  // `reason === "lost"`, refuses a render that flattens a verse. `alignment_edit`
  // stays exempt: unaligning everything from the aligner panel is legitimate.
