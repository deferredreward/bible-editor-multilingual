// checks.ts — deterministic QA checks for translated resources.
//
// Ported verbatim from bp-assistant src/lib/translate-checks.js (issue #445).
...
// pass explicit sets.
export const TN_PASS_THROUGH_COLUMNS: readonly string[] = TN_COLUMNS.filter((c) => c !== "Note");
export const TN_TRANSLATE_COLUMNS: readonly string[] = ["Note"];
// Back-compat export (was TN_COLUMNS.filter(...) in the tN-only version).
export const PASS_THROUGH_COLUMNS = TN_PASS_THROUGH_COLUMNS;
...
// corruption. Same rule the editor enforces via web/src/lib/hebrew.ts nfc().
export function nfc(s: unknown): string {
  return String(s ?? "").normalize("NFC");
}

export function extractRcLinks(s: unknown): string[] {
  return (String(s).match(RC_LINK_RE) || []).map((x) => x.replace(/[).,;]+$/, ""));
}

function extractAll(re: RegExp, s: unknown): string[] {
  const out: string[] = [];
...
function violation(check: string, severity: Severity, rowId: string | null, message: string, column?: string): Violation {
  const v: Violation = { check, severity, rowId, message };
 ...
 */
function checkTextColumn(source: TsvRow, target: TsvRow, col: string, { rowId, multi }: { rowId: string | null; multi: boolean }): Violation[] {
  const v: Violation[] = [];
  const src = source[col] ?? "";
  const tgt = target[col] ?? "";
  const tag = (base: string) => (multi ? `${base}-${col.toLowerCase()}` : base);
...
export function checkRow(
  source: TsvRow,
  target: TsvRow,
  { passThroughColumns, translateColumns }: { passThroughColumns: readonly string[]; translateColumns: readonly string[] },
): Violation[] {
  const v: Violation[] = [];
 ...
 */
export function runChecks(sourceRows: readonly TsvRow[], targetRows: readonly TsvRow[], opts: CheckOpts = {}): CheckResult {
  ...
  const tgtById = new Map<string, TsvRow>();
...
export function runArticleChecks(
  ...
  const headings = (s: string) => (String(s).match(/^#{1,6}\s/gm) || []).length;
