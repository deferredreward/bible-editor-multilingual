...
2026-07-20 · **worktree-translate-templates** — **Note templates become a stored, translatable, drift-tracked resource (backend; PR #56, draft).**
- Templates were a read-only Google Sheet proxy (`api/src/noteTemplates.ts` → the TEMPLATE dropdown in `NoteCard.tsx`), English-only, with nowhere to put a translation and no way to notice when the English content team reworded one you'd already translated. Now stored in D1 and treated like `article_units`: `template_units` (source_md/source_hash/target_md/translation_state/version/pre_draft_json), append-only `template_source_history`, and a `template_sync_state` watermark (migration `0054_template_units.sql` — renamed from 0053 after `0053_common_issues.sql` landed on main mid-branch).
- **Drift detection is the point.** A changed English body appends a history row, bumps `version`, and demotes `ai_draft`/`validated` → `edited` with `stale_source` + `prior_source_hash` in `draft_meta_json`. Rows that vanish from the sheet are soft-deleted (`deleted_at`), never dropped, and restored if they reappear. `GET /api/templates/unit/history` returns English source revisions and edit_log target revisions as **two separate lists** so the existing word-diff UI can render either.
- **Identity comes from a new column D (`id`) in the sheet**, which Benjamin (he's on the English content team) is adding via an Apps Script that stamps a frozen literal ID per row — survives reordering *and* rewording, re-issues on row-copy duplication, never reuses a retired number. **Hard dependency by design:** no `id` header in column D → the sync writes *nothing* and warns. The first cut fell back to a positional `${ref}-p${n}` id; the review pass killed that as a data-loss path — positional rows are translatable, and the moment real ids appeared every one would soft-delete and re-insert untranslated, orphaning interim work. Inert-until-the-column-exists is strictly safer, so the PR can still merge in either order; it just does nothing until the sheet is stamped.
- **The two quick-fill buttons are translatable too.** `TCM` and `buildSH()` are hardcoded English strings in `web/src/lib/noteTemplates.ts`, not sheet rows, so they're synced in as `origin='builtin'` units (`builtin-tcm`, `builtin-sh`; new `origin` column, CHECK sheet|builtin). Built-ins and sheet rows are planned as two independent diffs against disjoint slices of `template_units` — otherwise the sheet's soft-delete pass would wipe the built-ins on every run, since they are by definition absent from the sheet. Built-ins seed even when the sheet parse aborts. Sync is now 193 units (191 + 2). Note the sheet's `This could mean-01` row is NOT the TCM button — it's an unrelated worked example about Yahweh.
- **Decided (Benjamin, 2026-07-20): `builtin-sh`'s `{book}` placeholder resolves to the team's OWN book name, defaulting to the current book — in English too.** The source is `book_usfm_meta.headers_json`, which already stores each book's USFM `\h`/`\toc*` headers per `bible_version` (written by `extractUsfmHeaders` in `api/src/importParsers.ts:886`, stored in `bookImport.ts:510`). So: read the target Bible version's `\h` for the current book, fall back to `web/src/lib/bookNames.ts` (English-only) when there's no target header yet. Frontend work — belongs to the UI PR, not #56.
- Sync runs on the existing `POLL_CRON`, throttled to 6h, in its own try/catch so it can't take pipeline polling down. `GET /api/note-templates` is deliberately unchanged (same shape, same edge cache) — serving translated bodies is a later PR.
- Files: `api/migrations/0054_template_units.sql`, `api/src/templateSync.ts` (+`.test.mjs`), `api/src/templates.ts`, `api/src/index.ts`, `api/src/noteTemplates.ts` (exported `parseCsv`/`SHEET_CSV_URL`/`fetchSheetCsv` rather than duplicating the parser a third time), `api/src/types.ts`.
- **Verified — how:** typecheck clean; api 119/119 (11 new, covering `planTemplateSync`); all 54 migrations apply from scratch to a local D1. Then every route driven over real HTTP against `wrangler dev` + local D1: list/read 200, PATCH without If-Match 428, stale If-Match 409 `version_mismatch`, correct If-Match 200 (version bumps, → `edited`), validate 200 (→ `validated`, version unchanged, pre-draft snapshotted), history 200 with both lists.
- **Not verified:** the sync has never run against the real sheet (column D doesn't exist yet), so the column-D read path and the positional fallback rest on unit tests, not live data. First thing to exercise after the Apps Script backfill.
- **Blocked / next:** PR 2 = Templates tab in `ArticleWorkspace` + the diff view. PR 3 = AI translate (bulk + individual) and it is **blocked on bp-assistant**: `resourceType` is a closed enum forwarded to the bot (`api/src/pipelines.ts:116` — `tn|tq|tw|ta`) and the bot resolves a *DCS repo* per type, which a Google Sheet doesn't have. The bot needs a `template` type first, and since its `OptionsSchema` is `.strict()`, bp-assistant must deploy **before** bible-editor or every run 400s.
...
- **Shipped.** New migration `0054_book_tq_source.sql` (`book_imports.tq_source`, mirroring 0050's `tn_source`). `dcsSources.ts`: `LaneRepoOverrides` → `DcsRepoOverrides` with optional `tn`/`tq` refs honoured by `dcsUrls`; new pure helpers `sourceProvenance`, `translationSourceRepoRef`, `heldOutNoteResources` (the single shared "don't sync this from the org repo" predicate, unifying Aquifer + source provenance), and `shouldFallBackOnStatus` (404-only). `bookImport.ts`: optional body `{translateFromSource, force, confirmDiscardEdits}`, per-resource provenance stamped on the `book_imports` marker, watermark seeding passes the true upstream identity. `bookReimport.ts`: `resourceSourceRef` gained an override param; the reimport hold-out now uses the shared predicate. `exportWorkflow.ts`: the `aquiferTnBooks` step generalized to a `BOOK:resource` held-out set covering tn **and** tq. Frontend: opt-in checkbox in `SetupWizard`, an admin "Re-source notes from English" item with two-step confirmation in `PipelineMenu`, provenance surfaced after automatic fallback in `TopBar`, 17 new i18n keys across 14 locales.
- **`force` was necessary and is not scope creep.** The feature was initially unreachable for the very books that need it: `POST /:book/import` short-circuits with `alreadyImported: true` and **no API route anywhere deletes a `book_imports` row** (only an out-of-band dev SQL script that wipes all books). The Door43 re-import dialog can't help either — once a book carries provenance, `heldOutNoteResources` removes tn/tq from the reimport. So `force` (admin-only) bypasses both early-return branches and reuses the existing wipe-and-load path, guarded by a `409 has_local_edits {tn, tq}` refusal unless the caller also sends `confirmDiscardEdits`.
- **Pre-existing bug fixed in passing:** `planAndStageBookResources` (the chunked path the nightly cron actually uses via `runChunkedReimport`) had **no provenance check at all** — only `runReimport` did. So the nightly self-heal would have clobbered Aquifer-sourced tN rows today, independent of this feature. Both paths now share one predicate.
- **Verified — how:** local `wrangler dev` on an isolated port + `--persist-to "C:/wt-tnsrc"`, `ar-bsoj` preset, MAL. Plain import → 195 rows, `tn_source` NULL. Forced source import → 252 rows, `sources.tn = "source:unfoldingWord/en_tn"`, `forced: true`. The decisive check was done by independently downloading `unfoldingWord/en_tn/tn_MAL.tsv` and computing set overlap against the actual `tn_rows.id` values in D1 — **2.6% before, 100.0% after, 0 stale BSOJ rows left**. Negative paths driven live: `force` as an editor → 403 `forbidden`; `force` with one edited row and no acknowledgement → 409 `has_local_edits {tn:1, tq:0}`.
- **Hold-out verified at runtime, not just read.** "Nothing changed" is weak evidence, so a sentinel corruption was planted in one *pristine* row of MAL (held out) and of OBA (NULL-provenance control) — if tn is really processed, DCS restores it; if held out, it survives. Result on both the interactive `POST /:book/reimport` and the nightly chunked path: MAL's sentinel **survived** and every tn/tq counter was 0, while OBA's was **restored from DCS** (`tn: {updated: 1}`) by the identical call. The chunked path was driven as a real local Workflow (`curl "/cdn-cgi/handler/scheduled?cron=0+8+*+*+*"` — note `/__scheduled` returns the SPA because the assets handler claims it), with MAL's and OBA's `book_resource_syncs` watermarks deleted first so the SHA gate could not be the reason for the skip. MAL got **no new watermark row**, independently confirming "never fetch, never watermark" rather than fetch-then-discard. Export hold-out proven at query + predicate level (`heldOutNoteResources` fed the real D1 rows → `['MAL:tn','MAL:tq']`); no export render was run. `npm run typecheck` clean; api 108/108 (18 new dcsSources assertions); web 14/14; `check-i18n` gains zero new failures (same 6 pre-existing).
- **Honesty ledger — not verified:** the automatic 404 fallback path was never reached at runtime (MAL exists in BSOJ/ar_tn), so only the explicit opt-in is execution-proven; `shouldFallBackOnStatus` is unit-tested in isolation. The `force` safety gate's two `COUNT(*)` queries have no unit coverage — DB-bound, and this repo has no HTTP/D1 route harness. Book choice matters when re-testing: OBA's BSOJ and uW ID sets are **identical** (153/153), so it demonstrates nothing; use MAL.
- **Tripwire:** provenance is deliberately two nullable columns (`book_imports.tn_source` / `tq_source`), not a table, because tWL is out of scope for this mechanism and `book_resource_syncs` can't express it (that table is a multi-row watermark log, not a current-source-of-truth flag). If a **third** resource ever needs this kind of provenance, migrate to a `book_resource_provenance(book, resource, source)` table rather than adding a `twl_source` column.

2026-07-20 · **cool-hermann-43143b** — **Root-caused & fixed the recurring "main checkout wiped" disaster (worktree junction footgun).**
- **Symptom (recurring):** main's working tree periodically showed every tracked file ` D` (deleted) and an empty `node_modules`, breaking `npm install`/build/deploy from main. **Cause:** the old `worktree-init.ps1` junctioned `node_modules` (and `web/api` node_modules) from main into each worktree. On Windows a recursive delete of a worktree (`rm -rf` / `Remove-Item -Recurse` / `git worktree remove --force`) FOLLOWS the junction and deletes main's real files — main's `node_modules`, and transitively via npm's `node_modules/@bible-editor/{web,api}` workspace links, main's `web/` and `api/` **source**. So a routine worktree teardown wiped main. Evidence: main reflog + `git fsck` clean (not a git op / not a tracked script); junction topology across ~20 worktrees all pointing `…/node_modules → MAIN/node_modules`; Pattern-B worktrees prove npm always creates the `@bible-editor` source links. Recovery: `git restore .` in main (no uncommitted work lost) + `npm install`.
- **Durable fix (PR):** `worktree-init.ps1` now runs a real cache-fast `npm install` (self-contained worktree, no junction to main — ~314 MB each, negligible vs. free disk). New `scripts/worktree-cleanup.ps1` classifies worktrees SAFE/GRAY/KEEP and safely removes them — **unlinks junctions first** (`[IO.Directory]::Delete(path,$false)` = link only; verified end-to-end it leaves the target intact) before any recursive delete. Local nightly scheduled task `bible-editor-worktree-cleanup` (04:01) auto-removes merged+clean worktrees idle past a **72h grace** and flags the rest (uncommitted/unpushed/recent/orphan) for review.
- **Durable lesson (see Lessons learned):** never `rm -rf`/`Remove-Item -Recurse` a worktree that has node_modules junctions to main — unlink the junction first, or the delete cascades into main. Teardown must go through `worktree-cleanup.ps1`.
- **Not verified:** the nightly task's first live 04:01 run (created this session, not yet fired); recommend a manual "Run now" to pre-approve its PowerShell/git tool use. ~10 orphaned worktree dirs still carry live junctions to main until the first cleanup run clears them.

2026-07-20 · **worktree-prefs-common-issues** — **New "Common issues" section in Preferences & Memory, injected into every AI draft prompt.**
- Translators keep a per-language document of recurring problems (false friends, grammar traps, formatting habits — the immediate case is an Arabic one from a translator). There was nowhere to put it. Added `common_issues_md` to the `translation_prefs` singleton (migration `0053_common_issues.sql`, 50k cap vs. instructions' 20k since this holds a pasted document) with a fifth rail section between Instructions and Terminology, cloned from `InstructionsSection` (markdown textarea + preview toggle + the same If-Match/409/403 save handling).
- **Design call worth remembering:** it does NOT become a new context-pack file. Per `docs/CONTEXT-REPO-CONTRACT.md` §5 the bot MUST ignore pack files it doesn't know, so a `common-issues.md` would have been written and silently never read. Instead `renderInstructionsMd()` (`api/src/contextExport.ts`) concatenates `instructions_md` + a `## Common issues` heading + `common_issues_md` into the existing `instructions.md`, which the bot already injects verbatim into every drafting call. Additive prose change — no `format` bump. §7 of the contract now carries the open question of splitting it out once bp-assistant grows a reader (same shape as the pending `standards.md` split).
- Files: `api/migrations/0053_common_issues.sql`, `api/src/translationMemory.ts` (PrefsRow / DEFAULT_PREFS / zod / INSERT + UPDATE placeholders renumbered ?1..?10), `api/src/contextExport.ts` + `.test.mjs`, `api/src/exportWorkflow.ts` (SELECT), `web/src/sync/api.ts`, `web/src/components/PreferencesWorkspace.tsx`, `web/src/i18n/locales/en.json` (English only — `fallbackLng: "en"`), both docs.
- **Verified — how:** typecheck clean; api 108/108, web 14/14, `npm run build` green. Then driven in a real browser against a local wrangler + migrated D1: rail entry renders and routes, first save 200 (INSERT path, `translation_prefs` confirmed empty beforehand → version 1), reload persists, second save 200 (UPDATE path → version 2), and Instructions + Common issues both survive saving each other (versions 3/4, row read back directly from D1). That round-trip is the point — no unit test touches the real D1 write, and both statements' positional placeholders were renumbered.
- **Not verified:** the export side end-to-end. `renderInstructionsMd` and a `renderContextPack` regression case (instructions.md still emitted when ONLY common issues is set) are unit-tested, but no run pushed an actual pack to a `translation-context` repo and confirmed the bot consumed the new heading.
...
- Files: `web/src/components/PipelineMenu.tsx` (item + handler + `isAdmin()`/`api` imports), `web/src/sync/api.ts` (`aquiferDrafts` method + `AquiferDraftsResponse` type), 14 locale files (`pipeline.aquifer*` — English in en.json, English-placeholder in the 13 per convention).
- Verified end-to-end in-browser (local wrangler serving built SPA, dev-admin, ar-bsoj preset applied): menu item renders for admin+translation-project; clicking on un-imported PHM fired `POST /api/books/PHM/aquifer-drafts` → 409 `book_not_imported` → toast "Import PHM first, then pull Aquifer drafts." typecheck + 14 web tests green; `check-i18n` adds zero new failures (pre-existing `translation.stateAquiferDraft`/`sync.*` debt untouched).
- Branch cut from `origin/main` *after* PR #43 (org-detect) and the user-management PR merged; api.ts interface conflict (AdminUser vs AquiferDraftsResponse) resolved by keeping both. Merged latest main (smoke-test PR) before landing; STATE.md conflict resolved by keeping both entries.

2026-07-17 · **feat/onboard-translate-smoke** — **New additive Playwright smoke suite: empty DB → onboarding wizard → import → stubbed AI translate → approve, end-to-end.**
- New `tests/smoke/onboard-translate.spec.ts` proves the whole onboarding-to-translate contract in one serial test: dev-auth admin → drive the Setup wizard (detect org `BSOJ`, resolve the ar_avd/ar_nav ambiguous lit/sim candidates, apply custom-gl) → import OBA + populate articles → verify scripture/notes render → `POST /api/pipelines/start` (translate, stub bot) → poll to `done` → verify an `ai_draft` row with the `[AR-STUB]` marker → approve via the NoteCard → verify `translation_state=validated`. Negative dedup check (`already_running`) runs while the job is still non-terminal, not after (the server's dedup query only matches active states — see below). Fully separate from `tests/concurrency/`: own config (`playwright.smoke.config.ts`), ports (api 8797 / web 5175 / stub 8799 vs. 8787/5173), and `--persist-to` D1 (`C:/be-smoke` by default, `SMOKE_PERSIST`-overridable). **This is the (B) handoff prompt mentioned in the `scripture-translation-demo-unblock` entry below — completed independently in this branch, including the `reset-dev-content.sql` patch that entry flagged as "pending in the smoke-test branch."**
- **Three real bugs found only by actually running it (not by static review), all fixed within this PR's own files:**
  1. **Playwright starts `webServer` plugins BEFORE running the config's `globalSetup` file** (confirmed via `node_modules/playwright/lib/runner/index.js`'s `createGlobalSetupTasks` — plugin setup tasks come before the globalSetups map, not after). The original design (wipe+migrate the D1 in `global-setup.ts`) EPERMed every time because `wrangler dev` was already holding the persist-to SQLite file open by the time it ran. Fix: the wipe+migrate moved into `scripts/dev-smoke.mjs`, run synchronously before it spawns `wrangler dev`; `global-setup.ts` now just does a redundant idempotent migrations-apply as a sanity check.
  2. **The translate-apply path pins bot-supplied `rawUrl`s to `env.DCS_BASE_URL`'s origin** (`api/src/rawUrlPin.ts`, a deliberate anti-SSRF guard from a recent PR — not weakened). The stub's own `http://127.0.0.1:8799/stub/...` URLs failed the pin outright, so translate jobs silently applied zero rows even though IDs matched (confirmed by hand: `BSOJ/ar_tn` and `unfoldingWord/en_tn` share row IDs like `xm1w`/`jdr1` for OBA — this was NOT the task's flagged ID-mismatch risk). Fix: `dev-smoke.mjs` points `DCS_BASE_URL` at the stub's own origin, and `translate-stub-server.mjs` gained a reverse-proxy fallback (GET/HEAD only) forwarding anything that isn't one of its own routes to the real `git.door43.org` — so org detect / book import / manifest fetches stay real, while the stub's own rawUrls are legitimately same-origin.
  3. **MUI's `StepContent` Collapse only unmounts the outgoing wizard step after its exit transition finishes** — clicking "Next" twice back-to-back briefly resolved two "Next" buttons (strict-mode violation). Fixed by waiting for the button count to settle to 1 between clicks. Separately, navigating to the *same* verse hash twice (before vs. after translate) is a same-hash no-op (`page.goto` doesn't fire `hashchange`), so the SPA's chapter cache never refetched and the draft-chip assertion saw stale `v1` content — fixed with an explicit `page.reload()`.
  4. Also needed, unrelated to the above: a fresh worktree has no `web/dist`, but `wrangler dev` hard-requires it (`[assets].directory` in `wrangler.toml` is unconditional) — `dev-smoke.mjs` builds it once if missing. And back-to-back runs occasionally raced a still-closing prior `workerd` process on the persist-to file (port-free ≠ file-handle-free) — both the wipe and the migrate in `dev-smoke.mjs` retry with backoff.
- **Deviation from the task's literal step ordering (flagged for review):** the negative `already_running` dedup re-POST runs immediately after the first `pipelines/start` (while queued/running), not after the approve step — the server's dedup query (`pipelines.ts`) only matches jobs in `queued/dispatching/running/paused_*` states, so by the time a job is `done` a duplicate start would create a NEW job, not dedup. Verified: re-POST with the same scope while non-terminal returns the same `jobId` + `status: "already_running"`.
- MOD `scripts/translate-stub-server.mjs`: `GET /health` (readiness probe) + the DCS reverse-proxy above. MOD `scripts/reset-dev-content.sql`: added `DELETE FROM article_units` / `article_fetch_state` (org-scoped project data since migration 0050, not lexicon data as the old header comment claimed — it's counted by `hasLiveProjectData`'s `project_not_empty` guard) and fixed the header comment. MOD root `package.json`: `test:smoke` script. `.gitignore`: `playwright-report-smoke/`.
- **Verified — how:** `npm run test:smoke` green from a wiped `C:/be-smoke` (fresh-checkout equivalent) 4 times total, including two genuinely back-to-back runs with no gap (idempotence proof, both green after the retry-with-backoff fix). `npm run typecheck` clean. `npm run test:e2e` (concurrency suite) green afterward (10/10) — isolation proof; needed `BE_BASE_URL` override + a one-time `npm --workspace api run db:migrate:local` since this worktree's default-location local D1 had never been migrated (pre-existing gap, unrelated to this PR, not touched). Stub `--fail` path spot-checked by hand via curl: `state:"failed"` + `error` field, no 5xx — the editor API's poll handler is an untouched passthrough of that shape (did not additionally re-drive it through a live Worker).
- **Honesty ledger — not verified:** the UI wizard path is what actually ran in every green run observed (no run exercised the try/catch fallback to prove IT independently works); the fallback is code-reviewed, not executed, so if MUI's ambiguous-select behavior ever does destabilize, the fallback path itself is unproven in this session.

2026-07-17 · **claude/scripture-translation-demo-unblock-b59d07** — **Wizard org-detect fixed for public orgs; dev DB reset + migrated; Monday BSOJ demo prep.**
- **Bug fixed:** Setup wizard "Detect from org repos" failed with "Access to this org's repositories was denied" for **public** org BSOJ. Root cause (evidence-verified): `listOrgRepos` (`api/src/orgInference.ts`) attaches `DCS_SERVICE_TOKEN` to the Gitea listing call; that token is write-scoped and Gitea returns **403** for it on read endpoints (an *invalid* token 401s — tested; anonymous gets 200, org is public). Fix: on 401/403 with a token attached, retry the page anonymously; genuinely private orgs still return `dcs_forbidden`. +3 regression tests in `orgInference.test.mjs` (104 api tests green). Deployed to `bible-editor-api-dev`.
- **Dev environment:** remote `bible_editor_dev` was **4 migrations behind** (0050–0052 incl. aquifer `book_imports.tn_source` + article-populate schema) — applied. Then full content reset (`scripts/reset-dev-content.sql` + manual `DELETE FROM article_units; DELETE FROM article_fetch_state;` — the script keeps `article_units` but the `project_not_empty` guard counts it; patch pending in the smoke-test branch). Verified post-reset: cfg=0 v=0 tn=0 au=0 jobs=0 roles=11.
- **Demo book selection (live-verified against Door43/Aquifer):** BSOJ ar_tn is already majority-Arabic for *every* small NT epistle (all would show green); the only 100%-English-placeholder books are MAT/MRK/LUK/JHN/ACT/1-2CO/HEB + OBA/MAL. Decided: **PHM** (approved-green story; 93/93 Arabic, 100% en_tn ID overlap) + **MAL** (needs-review-teal story; 195/195 English placeholder, ch4=25 notes smallest; Aquifer arb has it). **MAL caveat:** ar_tn↔en_tn ID overlap ~2% (ar_tn built from an old en_tn), so AI-translate on MAL tN rows cannot correlate — aquifer matches by quote (unaffected); the live AI-translate demo runs on PHM/tQ.
- **Context export to DCS fails on dev** (prefs themselves persist fine in D1 `translation_prefs`): `context_repo_create_forbidden: token lacks repo-create permission for org BibleEditorService`. Export owner resolves to `DCS_EXPORT_OWNER=BibleEditorService`, and `BibleEditorService/translation-context` doesn't exist. **Benjamin-owned:** create that repo (initialized) if contextRef injection is wanted; until then translate runs raw baseline.
- Handoff prompts delivered to Benjamin for parallel agents on another account: (A) admin user-management panel over `user_roles`; (B) additive Playwright onboarding/translate smoke test with stubbed bot (includes the reset-script patch).

2026-07-17 · **feat/admin-user-management** — **In-app admin UI for the `user_roles` allowlist — no more raw SQL to grant/revoke editor or admin access.** (This is handoff prompt (A) from the entry above, done in parallel on this branch.)
- Why: `user_roles` (dcs_username/role/added_at/added_by) could only be edited by hand-running SQL against D1 — a real friction point for onboarding a new translator or promoting an editor to admin. This gives admins a REST API + Preferences panel to do it in-app.
- Shipped: (1) `api/src/adminUserRoutes.ts` (new) — `GET /api/admin/users` (LEFT JOIN `users` for `addedBy` display name, admins-first then NOCASE alpha); `PUT /api/admin/users/:username` body `{role}` — order: zod enum (admin/editor only) → username regex → DCS existence check (`GET {DCS_BASE_URL}/api/v1/users/{username}`, canonicalizes to DCS's own `login` casing, 404→`dcs_user_not_found`, network/5xx fails OPEN with `dcsVerified:false`) → last-admin guard (admin→editor demote refused via 409 `last_admin` when admin COUNT≤1) → upsert (`added_by` stamped only on first insert, preserved across role changes); `DELETE /api/admin/users/:username` — 404 if missing, same last-admin COUNT guard (covers self-removal), else delete (leaves the `users` DCS-account cache row untouched). All three routes `requireAuth`+`requireAdmin`. Mounted at `/api/admin/users` next to `/api/orgs` in `index.ts`. (2) `api/src/adminUsers.test.mjs` (new) — real Hono app + jose-signed cookies + fakeDb + stubbed DCS fetch; covers auth gating, last-admin guard (both PUT-demote and DELETE, count=1 vs count=2), and PUT validation ordering incl. DCS-casing canonicalization. (3) Frontend: `web/src/components/UserManagementSection.tsx` (new) — intro explaining the ~1h role-change freshness window (access-cookie TTL) and that read-only viewers come from Door43 org membership, not this table; add-user form; inline per-row role Select; `window.confirm`-gated remove; one snackbar mapping `last_admin`/`dcs_user_not_found`/`invalid_username`/`not_found`; unconditional refetch after every mutation attempt (success or failure) so the UI never shows a stale optimistic state. `sync/api.ts` gained `AdminUser` type + `adminListUsers`/`adminSetUserRole`/`adminRemoveUser`. `PreferencesWorkspace.tsx`: `"users"` added to `Section`/`ALL_SECTIONS`, an admin-gated rail item (same independence pattern as `setup`/`localization` — gated only on `role==="admin"`, not on `memoryAvailable`/`isTranslation`), and the pane branch. (4) i18n: `preferences.section.users` + `preferences.users.*` + `preferences.users.errors.*` added to `en.json` and mirrored as English-placeholder text into all 13 other locales (this repo's established convention — native review owed later, same class as prior `setup.*`/`localization.*` debt).
- Verified: `npm run typecheck` (api+web) clean; `npm --workspace api run test` 102/102; `npm --workspace web run test` 14/14; `node scripts/check-i18n.mjs` — same 6 pre-existing MISSING keys (`scriptureLanes.confirmWaiveBook`, `sync.*` ×4, `translation.stateAquiferDraft`) plus pt/ru's pre-existing plural-category gap on `setup.populateWarnings` — zero new failures. **Live curl matrix** (fresh-migrated local D1 via `wrangler dev`, dev-admin cookie): GET list showed the 11 seeded rows + devadmin; PUT real Door43 username `bcameron93` → 200 with DCS-canonical casing; PUT a fake username → 404 `dcs_user_not_found`; bad-chars username → 400 `invalid_username`; role `"viewer"` → 400 `invalid_body`; PUT/DELETE with no `X-CSRF-Token` → 403; demoted down to a sole real admin (`bcameron93`) → PUT-demote → 409 `last_admin`, DELETE → 409 `last_admin`; DELETE happy path → 200, repeat → 404; no-cookie → 401; editor-role cookie → 403. **Browser smoke** (chrome-devtools MCP, since Claude-in-Chrome MCP wasn't available this session): signed in as devadmin, Users rail item visible and wired to live data reflecting the curl-tested state; add-user form added `jessicaparks` as editor; inline role Select promoted her to admin; remove button's `window.confirm` dialog showed the correct interpolated username and removal succeeded; last-admin guard verified live in-browser (network tab showed `PUT .../bcameron93` → 409 `{"error":"last_admin"}`, and the row snapped back to "Admin" after the automatic refetch); signed in as editor (`christopherrsmith`) — no Setup/Localization/Users rail items shown, and navigating directly to `#/preferences/users` fell back to the default Project Mode pane exactly as specified.
- Honesty ledger — not verified: did not exercise the DCS-network-failure fail-open path (`dcsVerified:false`) live against a real network outage, only unit-tested; relied on the unit test's `globalThis.fetch` throw-stub for that branch. Native-speaker i18n review still owed for the 13 non-English locales (pre-existing debt pattern, not new to this PR).
- **PR:** opened as [#42](https://github.com/deferredreward/bible-editor-multilingual/pull/42) against `deferredreward/bible-editor-multilingual` (fork), not upstream. Flagged in the PR body: this touches all 14 i18n locale files, the most likely collision point with parallel worktrees also mid-flight on i18n.
- **Post-PR review fix:** both an independent workflow code review and a user-run Codex review flagged the last-admin guard as a TOCTOU race — the admin COUNT check and the role-change/delete write were two separate D1 statements, so two concurrent requests against the last two admins could each observe `count > 1` and both write, leaving zero admins. Folded the count check into the write itself (a single atomic `INSERT ... ON CONFLICT DO UPDATE ... WHERE` / guarded `DELETE ... WHERE`, using `meta.changes === 0` to detect a blocked write) so there's no read-then-write gap to race. Also applied three duplication cleanups from the same review: exported and reused `auth.ts`'s `lookupUserRole` instead of a bespoke inline SELECT; extracted the repeated `SELECT ... LEFT JOIN users` row-fetch into one `USER_ROW_SELECT` constant; extracted the frontend's duplicated role `<MenuItem>` options into a shared `ROLE_OPTIONS` array. Rewrote `adminUsers.test.mjs` to run against real `node:sqlite` (this repo's established D1-shim pattern, e.g. `projectConfigApply.test.mjs`) instead of a hand-rolled SQL-string-matching stub — needed because the atomic guard's correctness depends on real SQLite upsert/delete semantics; added a concurrency test that fires two simultaneous demote requests and asserts exactly one succeeds (102/102 api tests still green).

2026-07-17 · **claude/prefs-localization-editor-51993f** — **Localization editor tab in Preferences: edit UI strings live against English, server-stored, admin-only.**
- Why: the 14 UI-language JSON bundles ship at build time, so a native speaker who spots an awkward phrase had no way to fix it without a dev + release. This adds an in-app editor to shorten that loop to seconds and gather native-speaker polish. Decided lifecycle (dialogue with Benjamin): edits held **server-side** (not per-browser localStorage) so they spread org-wide on next load; an Export button downloads a drop-in merged locale JSON to commit back via PR; bundled files stay source of truth, overrides are a startup fast lane. **Admin-only**, edits the **currently-selected UI language** against read-only English.
- Shipped (9 files): (1) migration `0052_l10n_overrides.sql` — one row per lang, `overrides_json` nested `{ns:{key:text}}` bag + `version` for If-Match CAS (mirrors `translation_prefs` 0040). (2) `api/src/l10n.ts` — `GET /api/l10n/overrides` (no role gate, returns all langs so every reader picks them up at boot) + `PUT /api/l10n/overrides/:lang` (`requireAdmin`, `parseIfMatch`→428, first-write ON CONFLICT guarded on If-Match:0, UPDATE `WHERE lang=? AND version=?`→409, zod-validated recursive string bag, 512KB cap, lang-format 400); mounted in `index.ts`. (3) `sync/api.ts` — `getL10nOverrides`/`putL10nOverrides` + `L10nBag` type. (4) `i18n/index.ts` — added `react:{bindI18nStore:"added removed"}` so `addResourceBundle` re-renders live. (5) `i18n/overrides.ts` (new) — `applyOverrides` (deep-merge via addResourceBundle), `flattenEn` (leaf rows grouped by ns), `currentValue`/`bagFromFlat`/`flatFromBag`/`mergedLocale` (export = `getResourceBundle`)/`placeholdersOf`. (6) `main.tsx` — fire-and-forget boot fetch → applyOverrides. (7) `PreferencesWorkspace.tsx` — `"localization"` in `Section`/`ALL_SECTIONS` (NOT `SECTIONS`), admin rail item next to Setup, main-pane branch before the isTranslation gate, new `LocalizationSection` (search-filter, ns-grouped rows, read-only English + editable RTL-aware field, whole-bag save = stored∪draft via CAS, live apply on 200, 409/403 handling, Export downloads `<lang>.json`, placeholder-drop warning). (8) `en.json` — `preferences.section.localization` + `preferences.localization.*` (intro interpolates `{{language}}`, `saveCount_one/_other`).
- Verified: `npm run typecheck` (api+web) + `npm run build` clean. **Backend curl guards** (fresh migrated local D1, dev-admin): 428 no-If-Match · 200/v1 first write · GET persists+reflects · 409 stale If-Match:0 · 200/v2 correct CAS · 400 invalid lang. **Browser e2e** (wrangler dev :8791 serving built SPA, cookie-minted admin, es): tab renders admin-only; intro shows "Español"; **boot-applied server override live in the editor** (`common.save`→`Guardar!`); live edit→"Save 1 change" enabled→click→server `common.save`=`GuardarLIVE` v2→v3, draft cleared, button re-disabled; **hard reload re-seeds field from server** (full round-trip = org-wide proof); **Export intercepted** = `es.json`, 43KB full merged locale containing the override.
- Honesty ledger — NOT verified: (a) **403 non-admin** — dev mint auto-grants admin, so I couldn't mint a non-admin token; relying on the shared `requireAdmin` middleware (same gate proven on PUT /prefs etc.). (b) **placeholder-drop warning UI** — logic present, not clicked through in-browser (simple, low risk). (c) other 13 locale files lack the new `preferences.localization.*` / `section.localization` keys → English fallback (verified: es rail showed "Localization" in English) — intended optional follow-up, same class as prior `setup.*`/`detectOrg.*` i18n debt.
- **Dev-fork constraint (flagged to Benjamin):** built + verified on the **dev** worker + **dev** D1 only. Rolling the shared override store to **production** is a separate deploy decision that is Benjamin's call — prod/upstream untouched per standing rules.
- **PR [#39](https://github.com/deferredreward/bible-editor-multilingual/pull/39)** opened (rebased onto `origin/main` @ `9bbcdaf`). **Pre-merge review protocol clean in 2 Codex calls:**
  - Claude high pass fixed: (1) the new `preferences.localization.*` + `section.localization` keys were en-only → `check-i18n` reported 14 MISSING/locale; added English-placeholder copies to all 13 non-en locales (repo convention). (2) removed unused `preferences.localization.save`. (3) collapsed `saveCount` from `_one`/`_other` to a single interpolated key so it doesn't introduce CLDR plural-category gaps across complex-plural languages. Remaining `check-i18n` failures (`sync.*`, `setup.populateWarnings`, `scriptureLanes.confirmWaiveBook`, `translation.stateAquiferDraft`) are pre-existing on main, out of scope.
  - Codex gpt-5.5 (1 finding, REQUEST_CHANGES): `GET /api/l10n/overrides` had no auth gate — anonymous read of all overrides, contradicting its "any authed user" comment. Fixed with a `currentUserId` → 401 guard. Low real sensitivity (UI strings already ship in the public bundle) but the code now matches its contract. Codex gpt-5.4-mini verify: **FIXED, no regression** (auth middleware still precedes the route; boot fetch is fire-and-forget).
  - Verify caveat: anon→401 confirmed live; the authed happy-path (mint→GET/PUT 200) was proven on the first local server before the guard, and the guard only rejects the no-userId case (can't regress the authed path — `/api/auth/me`, untouched, behaves identically). typecheck+build clean after every fix.
...
- `custom-gl` preset added to `PRESETS` (`hidden: true`, all fields blank, `exportOwnerFromConfig: true`); GET `/presets` drops hidden presets from the selectable list but publishes a synthetic "current" entry (materialized from the ACTIVE config) when custom-gl is active, so the client Select always has a matching value.
- `api/src/projectConfigApply.ts` (new — factored out of the route for node:sqlite testability): custom-gl completeness + `isIdent` guard (org/exportOrg/all seven repos/nested translationSource repos, explicit translationSource object-or-null, lit≠sim); override-clearing on preset switch (`resolveOverridesIntent` — a different preset clears stored overrides unless the PUT explicitly supplies new ones, same-preset PUTs keep the existing 3-intent merge); empty-project tenancy guard on EVERY effective org change (`hasLiveProjectData` — any live rows in tn/tq/twl_rows, verses, or article_units → 409 `project_not_empty`, zero writes); **atomic config+lane commit** — both lane plans built first (full `configHash` diff, reusing PR #28's `configHash`/`planLaneReconcile`/`desiredLaneConfig` rather than parallel logic), then config row write + both lanes' fenced CAS updates commit in ONE `env.DB.batch`. Migration `0051_config_lane_atomic_guard.sql` adds `_abort_guard` (a poison-pill table with `CHECK(1=0)` — any row inserted into it unconditionally violates the constraint), used as `INSERT ... SELECT ... WHERE NOT EXISTS(<lane CAS still holds>)`: when a lane's fenced UPDATE silently matched zero rows (concurrent lease/job/revision race — D1 would otherwise happily commit the rest), the guard fires and rolls back the WHOLE batch including the config write. Same technique as PR A's `article_fetch_state` config-change fence.
- `PUT /api/project-config` (`projectConfigRoutes.ts`) rewritten atop `applyProjectConfig`; GET now publishes the hidden/synthetic-preset logic above.
- Web: `OrgDetectionSection` in `PreferencesWorkspace.tsx` (admin-only, under Project Mode) — org field + Detect → draft editor (verified roles prefilled, missing shown red, ambiguous → required select of candidates) → explicit translationSource toggle (default UW_SOURCE) + exportOrg field → Apply (disabled until complete) → `applyProjectOverrides("custom-gl", …)` → `refreshProjectConfig()`. 409 explains the one-org-per-database rule. `api.getInferredOrgConfig` / `api.putProjectConfigWithOverrides` added to `sync/api.ts`. i18n: `preferences.detectOrg.*` (14 keys) added to all 14 locale files (English fallback text in 12 non-English locales, matching this codebase's existing convention for not-yet-translated keys; real Arabic translation added since ar.json otherwise carries full translations).
- **Deviation from plan:** LaneCard's `initialUrl` prop (pre-filling the existing lane-replacement UI from the detected lit/sim repos) was NOT wired — for the empty-DB apply path (the only case actually exercised, since a populated DB is tenancy-guarded), the atomic apply installs the lane directly via `planLaneReconcile`'s `install` branch (verseCount===0), so the replacement-workflow UI was not required for this PR's scope. Changing lit/sim on an ALREADY-active custom-gl project still uses the existing LaneCard "paste a URL" flow unprefilled — a minor UX gap, not a correctness one.
- Tests: `api/src/orgInference.test.mjs` (15 cases — manifest fixtures incl. non-Bible-subject rejection and `?v=` stripping; inference incl. avd/nav ambiguous-fallback, multiple-`*_tn` ambiguity, missing tw/ta). `api/src/projectConfigApply.test.mjs` (28 cases, real node:sqlite schema incl. migration 0051 — custom-gl guards, override lifecycle, empty-project guard incl. TQ/TWL-only data and named→named org change, and both required atomic-rollback scenarios: a lease acquired on the second lane strictly AFTER planning, and a lane already busy pre-check — both assert the config row AND both lane rows are byte-for-byte unchanged). `projectConfig.test.mjs` updated for the new preset.
- Verified: `npm run typecheck` (api+web) clean; `npm run build` succeeds (api tsc + web vite build); api test suite 82/82 (`node --experimental-strip-types --no-warnings --test "src/**/*.test.mjs"`); web test suite 13/13 (`npm --workspace web run test`). Did NOT run a browser smoke test per the calling agent's instructions (orchestrator handles that).
- `js-yaml` + `@types/js-yaml` added to `api/package.json`; root `npm install` (no prior node_modules existed in this worktree, so no junction had to be replaced).
- Code review (`/code-review --fix` + Codex gpt-5.5 → gpt-5.4-mini ×2). Fixes landed: (1) `parseManifestFacts` dropped `dublin_core.language.{title,direction}` → now parsed from the tn manifest and preferred over the repo-name code / hardcoded RTL heuristic; (2) **P1 tenancy** — the empty-project guard only compared `org`, missing a custom-gl PUT that keeps `org` but repoints `exportOrg` or a non-lane repo; now compares the full effective data/export identity (`dataExportIdentity`: org + resolved export owner + tn/tq/twl/tw/ta repos), lit/sim excluded (lane flow's job); (3) **P2 starvation + hard cap** — `selectCandidateRepos` (extracted, pure, tested) reserves standard lane names before filling and hard-slices to the cap so it's a real subrequest ceiling. Round-3 finding (fence-snapshot order in PR A's `populateReferencedArticles`) was a **verified false positive** — line 576 reads the snapshot BEFORE `fetchAll` at 577, and the passing config-fence test (mutates config mid-fetch, asserts abort) proves the race is caught. **Follow-up owed:** `preferences.detectOrg.*` still English in the 12 non-Arabic locales — deliberately NOT machine-translated in review (unverifiable complex-script output is worse than English fallback); needs a native-reviewed i18n pass to reach PR A's parity.

2026-07-16 · **feat/tw-ta-article-population (PR #31)** — **tW/tA translate areas populated from a book's imported notes (multi-org plan, PR A of 3).**
- Problem: new orgs opened the tW/tA article workspace to a blank list — `article_units` was seeded only by the manual whole-corpus `scripts/import-articles.mjs` (hardcoded unfoldingWord/en_tw|en_ta).
- Shipped: deferred, self-draining reconciler `api/src/articlePopulate.ts` (no queue table). Triggers = `waitUntil` on every successful `POST /:book/import` path + gated reimport, POLL_CRON backstop (own try/catch), and `POST /api/articles/populate` (UI loops until `remaining===0`). Scans tn `SupportReference` + inline `rc://` note links + twl `TWLink`; fetches only referenced articles from `translationSource`; identity-aware upsert (new `source_org`/`source_repo` cols, migration **0050**) re-stamps on org change even when bytes match, bumps version only on real sha change, demotes stale validated/ai_draft→edited, never touches target_md/deleted_at/pre_draft_json. `article_fetch_state` (source-scoped: 404 terminal per source, error cap, void-on-source-switch). Write-time config fence = CHECK-violating guard INSERT rolls back the whole batch if translationSource changed since planning (snapshot captured BEFORE fetch in both reconciler and manual add). `fetchTextWithStatus` (strict 200/text/!truncated). `exportOwnerFor` + `exportOwnerFromConfig` capability (set on en-bible-editor-ml-test) routes a per-config org's exports to its own org, not env `DCS_EXPORT_OWNER`. PATCH: first edit of a NULL-state populated article → edited (so it can be approved). Manual add (`POST /api/articles/:resource/add`, restores soft-deleted) + refresh (cursor-paged) + search UI; `TwArticleDialog` reads from translationSource; i18n ×14.
- Verified: `articlePopulate.test.mjs` runs real migrations 0039+0049+0050 via node:sqlite (upsert matrix, fence rollback incl. manual-add mid-fetch, fetch-state machine, refresh cursor). Full api suite exit 0, web 13/0, typecheck+build clean. Review protocol clean: `/code-review --fix` (fixed a literal NUL-byte map-key delimiter that made the file diff as binary + false "populated" message on abort) → Codex gpt-5.5 (fence-snapshot timing in manual add → fixed+test) → gpt-5.4-mini (empty-workspace Add unreachable → fixed) → gpt-5.4-mini **no findings**. Browser smoke passed: empty-state populate filled 139 tW + 53 tA from ZEC refs, search+Add on empty and populated, tW dialog sourced from unfoldingWord/en_tw, idempotent re-populate (API layer).
- Stacked follow-ups: **PR B** (manifest inference) branched off this as `feat/manifest-inference`; **PR C** (per-user work mode) queued after B. Onboarding-wizard suggested task created, gated on PR B merging.
- Caveat (not PR A): fresh `POST /books/TIT/import` in the ML-test preset is blocked by the lanes' `replacement_required` state (unrelated scripture-lane workflow) — smoke used seeded ZEC instead.

2026-07-16 - **claude/project-setup-issues-c0ec37 (PR #29)** - **Code review pass on the meta_missing_for_book fix; narrowed snapshotRequiredBooks to verses-only.**
- Review (8 independent finder angles + verify) surfaced a real refinement: unioning `book_usfm_meta` into "required books" (the first version of this fix) would have let a stray meta-only row (no matching verses) become a phantom required book, forcing the new source to supply/waive content that was never really part of the lane. `verses` is the sole authoritative "what's active" signal; meta is enrichment written alongside it, not an independent membership signal.
- Fix refined: `snapshotRequiredBooks` (scriptureLane.ts) now queries `verses` only, dropping the `book_usfm_meta` query and the union entirely -- simpler, one fewer D1 round-trip, and no phantom-book side effect. Regression test's meta-only-book case (OBA) now asserts EXCLUSION, not inclusion.
- Also: removed the two `meta_missing_for_book`/`verses_missing_for_book` i18n codes from `LANE_ERROR_CODES` + all 14 locales (unreachable dead weight - server can no longer emit them) and added the real, reachable `activateReplacement` error codes (`job_not_found`, `job_not_ready`, `export_lease_held`, `export_lease_grace`, `activation_cas_failed`) with translations, since `handleActivate`'s catch block was the other site this fix touched but hadn't covered. Fixed a `book !== undefined` vs truthy check bug (empty-string book segment was silently dropped from interpolation) and simplified the code-parsing split from a regex to plain `indexOf`.
- Verified: api unit tests 35/35 (incl. updated regression); web typecheck clean; all 14 locales valid JSON + `errors.*` at full i18n parity (same 5 pre-existing gaps as before, untouched).
- PR note: this PR was first mis-opened against `unfoldingWord/bible-editor` (upstream) instead of the fork -- `gh pr create` defaulted to upstream. Closed there, reopened correctly as `deferredreward/bible-editor-multilingual#29`. CLAUDE.md now documents this gotcha.
...
- **Safety rails (task item 3):** shrink-guard analogue rejects a render that would drop >5% (and >25) of the files **we last successfully exported** for this dir — the truncated/partial-D1 signature, the article carry-over of the twl_PSA clobber backstop. **Baseline is our own `export_snapshots` history (`commit_sha IS NOT NULL`), NOT the target repo's live .md count** — deliberately, because a GL `_tw`/`_ta` repo is commonly provisioned by seeding the full English article set, so comparing against its live file count would false-block every early export (few translated vs ~1,000 English files). This baseline directly measures "previously-exported by us," matching the task's wording, and keeps the guard pure-D1 (no DCS read). **Freshness gate:** the verse path's SHA-watermark "master moved ahead of D1" gate has **no article analogue and is deliberately N/A** — there is no article DCS→D1 reimport loop (`import-articles.mjs` only seeds `source_md`; no watermark table) and the GL target has no out-of-band writer (only this export writes it, via a PR). Documented in-code (`exportArticleDir` / `lastArticleExportCount`) so it's not a hidden gap.
- **Known v1 limitation (by design):** the export is **additive** (Gitea ChangeFiles create/update only, never delete) — matching design §5's "export whatever exists" WIP model. Clearing a `target_md` back to NULL or soft-deleting an `article_units` row removes it from the render but leaves the previously-exported file stale on the target branch; pruning removed articles is left to the future gl-publisher (it owns release-shaping). This is why the shrink guard's job is to *detect + alert* on a suspicious shrink, not to gate a deletion (there is none).
...
2026-07-14 · **feat/ui-localization-reconciled** — **Rebased/reconciled the 4-wave i18n stack onto advanced main + landed 2 code-review fixes. PR #13 → main (supersedes #4/#6/#8/#11).**
Merged current `origin/main` (10 commits ahead, incl. merged PR #5: Words→TWLinks rename + tQ/tW/tA translate series) into a single consolidated branch. **Conflict surface was tiny:** `en.json`/`ar.json`/`Shell.tsx` all AUTO-merged (main's `shell.words`→"TWLinks" took cleanly since my side never edited that line; my new namespaces landed in different regions); only `STATE.md` needed a manual append-both resolve. Reconciled: (a) propagated the TWLinks rename to my `lanes.tw` label + `lanes.twShort="TWL"` across all 13 locales; (b) translated main's new `articles` namespace (15 keys) + 2 tQ `translation.*` keys into the 12 locales that lacked them (en/ar came from main) via `scratchpad/reconcile-main.mjs`. en now **465 base keys / 14 plural**. **Code-review fixes to `check-i18n.mjs`** (self + Codex review, commit `36954aa`): en-authoritative plural-base detection (drop the fragile `!flat.has` clause → a future `phase_one` won't be misread as plural; id/th `_other`-only still resolve) + a **code→en orphan scan** (`t("...")`-scoped) so key typos fail CI. Verified: checker 13/13 + no orphans, `npm run typecheck`/`build`/web-unit all green (the auto-merged Shell.tsx + main's new components compile together), and **e2e 10/10** — run on an isolated stack (vite `:5188` → wrangler `:8795` via `VITE_API_PROXY=http://127.0.0.1:8795`, fresh D1) to sidestep the peer's `:8787`. The recurring "e2e blocked" from earlier this session was purely that shared-port contention; **the fix is `VITE_API_PROXY` to a free wrangler port + a pinned vite port** (don't fight over :8787). **Second reconcile (merge `acfd278`):** main then merged PR #10 (issue #7's project-config resource labels) which conflicted on the 3 aligner/source files; resolved to keep BOTH issue #7's `versionLabel()`/`versionDisplay` (dynamic label) AND my i18n `t()` (surrounding words), parametrizing `aligner.openDualTooltip`/`dualSubtitle` to `{{left}}`/`{{right}}`/`{{source}}` across all 14 locales. Re-verified green (checker/typecheck/build/e2e 10/10). PR #13 now conflict-free. Commented "superseded by #13" on #4/#6/#8/#11 — Benjamin to close them. All translations still machine-drafted; native review owed.

2026-07-13 (cont.2) · **feat/ui-localization-wave4** — **Externalized board/lanes/logos/source + translated 13 locales. PR #11 (stacked on #8).**
Closes the spots reported after Wave 3: chapter-board page, lane labels + T/N/W/Q short abbrevs, Logos sync bar (brand "Logos" literal), aligner "source". `lib/laneChecks.ts` LANE_LABELS removed → `lanes.*` i18n keys; `laneAttribution` takes `t`. New `lanes`+`logos` namespaces + `shell`/`aligner` additions; en now **448 base keys / 14 plural**. Translated the 32-key delta into all 13 locales via a single Node merge script (`scratchpad/wave4-translate.mjs`) because the Agent classifier was down ~1.5h (couldn't fan out subagents). Commit `ed33515`. Checker 13/13, typecheck, build, web unit green.
**⚠️ e2e NOT cleanly run:** a parallel worktree session (issue #7) holds the shared `:8787` wrangler port, so the concurrency suite's WS-view tests (s6/s8) hit the wrong backend. **Confirmed environmental, not this diff:** stashed Wave 4 and re-ran s6 on the committed Wave-3 tree → identical failures. Static gates + checker are the reliable signal here.
**⚠️ MAIN ADVANCED 10 commits — REBASE/RECONCILE NEEDED before merging the stack.** New main includes merged PR #5: `93dc15e feat(i18n): rename Words resource tab to TWLinks`, plus a tQ/tW/tA translate-pipeline series (`0e8ad38`, `f9724bd`, `74f4579`, …). These overlap my 4-PR stack (#4/#6/#8/#11) on `en.json`, `NoteCard.tsx`, and resource/label naming. Notably the **"Words"→"TWLinks" rename** likely conflicts with my `words.*` / `lanes.tw="Words"` — my locales may need "TWLinks" naming reconciled. The whole stack is based on old main (`db15242`); rebase all four onto current main and resolve en.json + NoteCard conflicts, reconciling the TWLinks rename, before merge.
...
2026-07-14 · **beautiful-colden (branch `feat/translation-preferences-panel` off `main`)** — **Translation Preferences & Memory panel: Phase 1 research + Phase 2 design + Phase 3 build (backend + frontend) complete; committed (`ccfa32b` backend/docs, `ea4fc40` frontend).**

- **Phase 1 [`docs/translation-preferences-research.md`]:** sourced industry survey (TMX/fuzzy-match conventions, TBX `normativeAuthorization` picklist, forbidden/DNT CAT-tool layer, ISO 17100 briefs, DeepL/Amazon formality enums, memoQ/Trados/Xbench/Phrase/Smartling/Crowdin QA taxonomies, MQM/DQF, LLM glossary/few-shot steering, Paratext Biblical Terms, uW GL Manual + tW). All claims cited with access dates; gaps flagged (MQM canonical page 403'd; no incumbent Bible-translation TM standard; uw-bt-bot steering undocumented).
- **Phase 2 [`docs/preferences-panel-design.md`]:** D1 = operational truth, `{org}/translation-context` git repo = derived export the bot reads via opt-in `contextRef` (the seam `translateOptions.ts:57-66` left open). Singleton `translation_prefs` (brief+instructions+register enum+assisted flag) mirroring `project_config`; concept-oriented multi-rendering `terminology` (TBX core statuses + forbidden→replacement + DNT); examples = existing `translation_state='validated'` rows (no new table); `qa_rules` config-only (checks in code); `note_templates` per-SupportReference. ⚠ `CONTEXT-REPO-CONTRACT.md` doesn't exist yet — the git-export leg is designed, NOT built.
- **Phase 3 backend [`ccfa32b`]:** migration `0040_translation_memory.sql` (translation_prefs, terminology, qa_rules, note_templates); `translationMemory.ts` router (prefs GET/PUT with If-Match CAS, terms CRUD + count + CSV import w/ dry-run + export, examples browse joining validated tn/tq rows); pure `translationMemoryLib.ts` (CSV parse/serialize, dedupe) + `translationMemoryLib.test.mjs` appended to the api test chain.
- **Phase 3 frontend [`ea4fc40`]:** `PreferencesWorkspace` (hash route `#/preferences/{section}`, GL-gated via `isTranslationProject`): Brief + Instructions (markdown preview) editing the singleton; Terminology CRUD with status chips + forbidden "use instead" + CSV import/export; Examples browse/search with revoke via existing `validate(0)`. `useTranslationMemory` hooks mirror `useArticles`; TopBar entry; the `ResourceColumn` language-memory chip's hard-coded `0 terms` now live. i18n en+ar full parity.
- **Verified — how:** `npm run typecheck` (both workspaces), api unit suite (incl. new lib tests), web unit suite, `npm run build` — all green. **NOT verified: in-browser smoke test** (the session's Bash safety classifier flapped for hours; dev-server run pending) and **no e2e against the real API routes** (no D1 test harness exists — route handlers follow the `articles.ts` template verbatim).
- **Browser smoke (in-app Claude Browser, worktree wrangler on :8788 serving the built SPA, `ar-bsoj`):** GL gate shows the explainer under English root; panel renders (rail + العربية); Brief save round-trip (audience persisted, version 0→1 CAS); term added via UI (kt/grace→نعمة); CSV import dry-run (verified no write) then apply — multi-rendering concept kt/lord (preferred الرب + forbidden السيد with replacement) + DNT names/yhwh → count 4; export round-trips with Arabic + tw_link intact; UI status chips correct incl. forbidden "use instead: الرب"; Examples tab correct empty state ("0 validated"); Instructions save persisted (version 1→2).
- **Local-dev traps hit this run (lessons):** a peer wrangler on :8787 serves stale main-code while claiming our `npm run dev` also "Ready on 8787" — probe `/api/translation-memory/prefs` to detect, pin a free port (`wrangler dev --port 8788`, serves SPA via `[assets]`); fresh worktrees lack `api/.dev.vars` (copy from main or `/api/auth/dev` 500s `jwt_signing_key_not_configured`); a prod-built SPA never auto-mints dev auth — mint manually via `POST /api/auth/dev` then reload; Claude-Browser coordinate clicks silently miss MUI buttons — use DOM-dispatched clicks + native value setters.
- **PR + review cycle:** opened [deferredreward/bible-editor-multilingual#14](https://github.com/deferredreward/bible-editor-multilingual/pull/14). `docs/CONTEXT-REPO-CONTRACT.md` added (repo layout + schemas for bp-assistant's `translate-context` reader, superseding PIPELINE-SPEC §3's sketch). `/code-review medium --fix` (8-angle) found 24 confirmed issues (case-mismatch CSV upsert, prefs-race, unescaped LIKE, missing `trashed_at` filter, silent UI failures, import-panel auto-close, literal-NUL binary file, several dedup/reuse cleanups) — fixed. 5 rounds of Codex review (gpt-5.5 then gpt-5.4-mini ×4) each caught something real the prior pass missed: forbidden-term replacement invariant never enforced, duplicate-term 500 instead of 409, a genuine TOCTOU race + viewer-role blank-panel gap, a viewer 403 on the terms-count chip, stale `replacement` persisting across a status change. **Round 5: approved, no remaining findings.** Every round re-verified with typecheck + api/web test suites + `npm run build` (all green throughout) before pushing.
- **Honesty ledger:** examples reach the AI only once the context-repo export + assisted mode exist (designed, deferred — blocked on the newly-written `CONTEXT-REPO-CONTRACT.md` being confirmed by bp-assistant); QA-rules + templates UIs deferred (backend tables exist); tW key-term seeding deferred; register/terminology actually reaching uw-bt-bot unconfirmed (their steering is undocumented); the Examples endpoint remains target-only (no English source join) — documented as a known gap in the route's own comment rather than silently implied-solved. Rebased onto `origin/main` (`20f4443`) post-review; only conflict was this file + `api/package.json`'s test-chain append (both resolved append-both, per this file's own convention). **⚠️ i18n gap surfaced by the rebase:** main's `scripts/check-i18n.mjs` (post-dates this branch) now fails on the new `preferences.*` namespace (~40 keys) — translated into en/ar only, missing from the other 12 locales (es, fr, hi, id, pt, ru, sw, ne, bn, ur, fa, th). **Not a CI blocker** (`.github/workflows/ci.yml` runs typecheck/web-tests/build only, not the i18n checker) — confirmed with Benjamin and deliberately left as a follow-up, matching this repo's established "Wave" pattern (translate a namespace's delta into all locales via parallel subagents) for whoever picks it up next.
...
2026-07-13 · **frosty-hypatia-15edae (worktree)** — **Manifest/config-driven resource display labels. Built + static-verified + in-browser verified. NOT pushed/PR'd.**
Made the ULT/UST/UHB/UGNT **display** labels project-config-driven while keeping the D1 `bible_version` role codes (ULT/UST/UHB/UGNT) and all logic/lookups/`READ_ONLY` sets unchanged. Scope chosen by the user: **structural labels only** (tier A) — headers, aligner strips, side-by-side subtitle, export dialog, quote-builder section labels. Prose i18n strings that name the codes (glossTooltip, "Generate ULT + UST", ultLabel/ustLabel, find tooltip, Shell errors) were **left for the `feat/ui-localization*` branches** to avoid conflicts.

**Backend:** `api/src/projectConfig.ts` gained `origHebrewLabel`/`origGreekLabel` (default "UHB"/"UGNT") alongside the existing `litLabel`/`simLabel`; added to all 5 presets + the `materialize()` override-merge. The GET `/api/project-config` `presets` summary array is unchanged (no labels there).

**Web:** mirrored the two fields on the `ProjectConfig` type in `web/src/sync/api.ts`. New pure helper `web/src/lib/versionLabels.ts` → `versionLabel(cfg, roleCode)` maps ULT→litLabel, UST→simLabel, UHB→origHebrewLabel, UGNT→origGreekLabel, **falls back to the role code** when cfg is null (first paint/offline) or older-schema (missing originals fields). Threaded via `useProjectConfig()` into the render sites (role code stays for `bibleVersion` props / lookups / `highlightsFor`):
- `ScriptureColumn.tsx`: version toggle (was `VERSION_LABEL[v]`), `StackedBody` active-row labels (ULT/UST + `uhbLabel`→`uhbDisplay`), `InactiveVerseRow` ULT/UST headers. NOTE: the module-level `VERSION_LABEL` identity map is now only referenced by `ActiveLine`'s leaf `{VERSION_LABEL[label] ?? label}` where `label` is already the resolved display string → harmless passthrough; left in place.
- `AlignmentPanel.tsx`: `InventoryStrip` "{version} words" header; `ActionBar` "editing {v}"/"Save {v}" + the "open ULT+UST side by side" tooltip (labels resolved, sentence kept). Labels resolved **inside those sub-components**, not the main forwardRef (they receive `bibleVersion` as a prop).
- `SideBySideAligner.tsx`: main subtitle (`{left}↔{right}·both aligned to {source}`), `ReadingLine` "{v} · reading text"/"Save {v}"/"no {v} text".
- `UhbStrip.tsx`: "{source} · source" header + show/hide tooltips (kept `sourceIsHebrew = sourceLabel === "UHB"` on the role code).
- `BookView.tsx` column header; `DocColumn.tsx` "{v} · editing/read-only" header.
- `ExportUsfmButton.tsx`: fed the resolved label into the existing `export.aligned`/`export.plainText`/`export.noText` `{{version}}` i18n templates (filenames still use the role code).
- `QuoteBuilderPopper.tsx`: source + ULT + UST `<Section>` labels (EmptyHint prose left as-is).

**Verified — how:** `npm run typecheck` (api + web, green), `npm run build` (api typecheck + vite build, green), `npm --workspace web run test` (green, incl. the new file) and `npm --workspace api run test` (green) — all run after the final edits. Added `web/src/lib/versionLabels.test.mjs` (pure-function coverage: role→label maps for en/GL, native-Arabic original, unknown-code passthrough, null-cfg + older-schema fallback) and wired it into `web/package.json` test script.
**In-browser (Claude in-app Browser + preview MCP):** seeded local `bible_editor_dev` D1 with ZEC (migrations + `import-book.mjs ZEC` + `d1 execute`), ran my own Worker on **:8788** and vite on **:5174** (`VITE_API_PROXY` → 8788) on free ports since a peer held 8787/5173. **English (`en-unfoldingword`):** scripture column, active card, align button, export dialog all render **ULT/UST/UHB**. Flipped D1 `project_config` to **`ar-bsoj`** (direct `d1 execute` + Worker restart to drop the 60s isolate cache + cleared the web `bible-editor.project-config.v1` localStorage key + reload) → the **same ZEC data** re-rendered with **GLT/GST** at every site: version toggle, active card + "align GLT"/"align GST", **all** inactive-row headers, export menu ("GLT — aligned" / "GST — plain text (no alignment)"), and the AlignmentPanel ("**GLT words**" word bank, "editing GLT", "Save GLT", "open GLT + GST side by side" tooltip). The **UHB source strip stayed "UHB"** (its default). Hebrew tokens + English text byte-identical across both configs → role codes intact underneath. Servers stopped after; shared 8787/5173 never touched.

**Honesty ledger — caveats:**
- All primary label surfaces were visually confirmed live (see above). The two surfaces I did **not** click into are the `SideBySideAligner` popup body and `QuoteBuilderPopper` — but the SideBySide *tooltip* ("open GLT + GST side by side") confirms its label wiring, and QuoteBuilder uses the identical `versionLabel` helper (unit-tested). Low residual risk.
- Left local worktree D1 with `project_config` = `ar-bsoj` and ZEC seeded; created `api/.dev.vars` (gitignored, dummy JWT key) + `scripts/out/import-ZEC.sql` (gitignored) — all local throwaway, none committed.
- `origHebrewLabel`/`origGreekLabel` default to "UHB"/"UGNT" for every preset (the originals are the universal unfoldingWord hbo_uhb/el-x-koine_ugnt); they're overridable per project but no preset renames them yet.
- Prose/tooltip strings that embed the codes (tier B) intentionally untouched — coordinate with `feat/ui-localization*`.
