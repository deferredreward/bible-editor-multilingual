...
2026-07-17 · **claude/bem-flexible-layouts-3d4b0a** — **Flexible layouts PR 1 (panel-decomposition foundation) — 6 commits, pushed, verified in-browser. Ready to open PR. Closes GAP prerequisite; the 3 gap features come next.**
- **Context correction:** this branch lives in the `.claude/worktrees/bem-flexible-layouts-3d4b0a` worktree (real node_modules — do NOT re-junction). The `busy-knuth-49b708` session was launched on a fresh main worktree by mistake; all work was done here on the correct branch. Merged `origin/main` in first (clean; main hadn't touched the panel files) → HEAD was `9ae9286`; now `30d0de4`.
- **Shipped (6 commits, each typecheck+web-15/15 green):** (1) `resourcePanelShared.tsx` — pure move of DropIndicator/VerseGroupHead/SectionHead/sortBySortOrder/groupByVerse + types out of ResourceColumn. (2/3/4) `WordsPanelBody`/`QuestionsPanelBody`/`NotesPanelBody` — content-only, props-driven panel bodies; ResourceColumn renders them inside its UNCHANGED scroll body so Classic stays byte-identical. NotesPanelBody takes `renderNoteCard` as a render-prop → ResourceColumn's delicate drag/reorder/flash/scroll-to-active machinery is 100% untouched. ResourceColumn 1559→1111 lines. (5/6) `StackedResourcePanel.tsx` + `renderRegion` rewrite: a multi-panel STACKED region (`display!=="tabs"` && `panels.length>1`) now renders each panel separately (reusing the *PanelBody components); scripture→scriptureNode; original/article/alignment/search→labeled placeholder. Shell's ResourceColumn props extracted to a shared `resourceColumnProps` object; ResourceColumn exports `ResourceColumnProps`.
- **Byte-identical guard (critical):** the stacked branch is gated on `display !== "tabs"`. **A real bug was caught in code review** (not by types/tests): the executor's first cut gated only on `panels.length > 1`, which would have flipped Classic's 3-panel tabbed resources region into 3 stacked panels — a Classic regression green gates DID NOT catch. Fixed + traced all 4 built-ins. Lesson: Classic fidelity MUST be eyeballed, not trusted to typecheck+unit-tests.
- **Browser-verified (live, ZEC in a local isolated stack):** Classic = tabbed Notes/TWLinks/Questions/Alignment/Search column intact + scripture modes + note/word cards render; tab-switch works. Book Package Review = nested layout (scripture columns-mode on top; res-a[Notes,tA] + res-b[TWLinks,tW,Questions] STACKED below), taArticle/twArticle placeholders shown, NO alignment/search tab strip (= new path, not the old collapse). translate-* correctly hidden on the English-root project. (Minor unverified: whether the stacked TWLinks panel's word ROWS render below its head — "read article" was absent; spot-check Monday. Not a Classic concern.)
- **DEFERRED to the 3 follow-on gap-feature PRs (user-agreed scope):** (a) movable panels (drag between/reorder/region-hide) + add Flexible·Columns/Nested builtins + drag-reorder inside stacked panels [feature 1]; (b) side-by-side source→target notes via `useSourceNotes`+`pairAxis` [feature 2]; (c) lightweight review-oriented scripture pane [feature 3]. Also deferred: the standalone original-language (Hebrew/Greek) panel (woven into ScriptureColumn; plan calls it a later extraction).
- **Dev-env verification recipe that WORKS on this machine (defeats the local-D1 MAX_PATH trap):** create `api/.dev.vars` (random JWT_SIGNING_KEY, gitignored); `wrangler d1 migrations apply bible_editor_dev --local --persist-to "C:/bem-verify"`; seed `node scripts/import-book.mjs ZEC` → `wrangler d1 execute bible_editor_dev --local --persist-to "C:/bem-verify" --file=../scripts/out/import-ZEC.sql`; run Worker `npx wrangler dev --port 8801 --ip 127.0.0.1 --persist-to "C:/bem-verify"` from api/; run vite `VITE_API_PROXY=http://127.0.0.1:8801 npx vite --port 5180` from web/ (vite DEV mode auto-mints dev auth). Browse `http://localhost:5180/#/ZEC/1/1`. **`--persist-to` short path on BOTH migrate+seed+dev is the fix** (deep worktree path → SQLITE_CANTOPEN / seed-invisible-to-dev). ONLY ZEC has a full local sample set (tn/tq/twl); OBA/MAL/3JN don't. In-app Browser: `read_page`/click work; **screenshots hang (30s timeout)** — use the a11y tree + `javascript_tool`. The seeded `C:/bem-verify` D1 + `api/.dev.vars` are LEFT in place for a fast Monday restart (just relaunch the two servers).
- **NEXT (Monday):** open PR 1 (branch pushed at `30d0de4`, `--repo deferredreward/bible-editor-multilingual`; rebase-check main first). Then build the 3 gap features on this foundation, in the user's order: movable panels+Flexible builtins → side-by-side notes → simpler scripture pane.

2026-07-17 · **claude/bem-flexible-layouts-3d4b0a** — **Flexible layouts: Phase 0 design locked (6-round mockup) + Phase 1 lib shipped (recursive-container v2 model). Rebased onto main (PR #31/#32). Building Phases 2–5.**
- Goal: Logos-style flexible layouts — named workflow presets + user-saved layouts; keep today's Shell as "Classic" (byte-identical, unchanged), add focused/simplified and whole-book-package views. Plan: `~/.claude/plans/take-a-look-at-toasty-hippo.md`. Mockup (6 layouts, runtime-verified): `docs/mockups/flexible-layouts.html`.
- **Durable design decisions:** (1) preset + slot system, NOT free-floating docking; `react-resizable-panels` v4 (RTL spike PASSED — set `dir` on a DOM ancestor; v4 API `Group`/`Panel`/`Separator`). (2) two-layer model: containers (recursive h/v split tree) vs pane arrangement (panels-per-region + per-pane config). (3) editability is a panel property, NEVER gated by layout (source read-only everywhere, target editable in all layouts incl. BP Review). (4) every region is a generic drop target (no type-locked regions); regions hide/restore (runtime). (5) bidi: content sets its own `dir` by text language (English `ltr`, Arabic `rtl`) — do NOT inherit workspace dir. (6) status dots grey=untranslated / amber=AI-draft-unapproved / green=approved. (7) server-shipped defaults + client fallback; personal saved layouts localStorage-first.
- **Phase 1 SHIPPED:** `web/src/lib/{layoutSpec,builtinLayouts,layoutStore,panelRegistry}.ts` + 2 `.test.mjs`. `LayoutNode=SplitNode|PanelRegion` + `PanelInstance/PanelConfig`, `LayoutSpec{v:2}`; strict recursive validator (depth cap 8); 4 built-ins (classic / translate-notes / bp-review-nested / translate-words); store `be:layouts.v2` + `LayoutOverride`+`mergeOverride`; registry (10 panel types) + `validateLayoutAgainstRegistry`. Verified after rebase: web typecheck clean, 15/15 web tests.
- **Phases 2–5 SHIPPED + always-show-switcher fix (all pushed).** `WorkspaceLayout` renders a LayoutSpec (Classic byte-identical flexbox path; others via `react-resizable-panels` v4); Shell `renderRegion` maps region→one component; TopBar `LayoutSwitcher`; `ResourceColumn` `visibleTabs`; server `builtinLayoutsFor` on `GET /api/project-config` + client fallback; save/manage user layouts (`LayoutMenu.tsx`). typecheck/build clean, web 15/15, api 96.
