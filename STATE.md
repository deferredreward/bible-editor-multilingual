...
- Code review (`/code-review --fix` + Codex): backend had no correctness bugs. Fixed one completeness gap — `parseManifestFacts` dropped `dublin_core.language.{title,direction}`, so a GL's language name fell back to the repo-name code and direction was guessed from a hardcoded RTL set; now parsed from the tn manifest and preferred (tests added). **Follow-up owed:** `preferences.detectOrg.*` still English in the 12 non-Arabic locales — deliberately NOT machine-translated in review (unverifiable complex-script output is worse than English fallback); needs a native-reviewed i18n pass to reach the parity PR A set.
