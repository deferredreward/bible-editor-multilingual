...
- **Review round — a high-effort review pass plus live re-verification found four more real problems, all fixed in-branch:**
  1. **Cross-org privilege escalation.** The `role` claim lives in the access JWT, so a workspace switch carried the previous org's role for up to the token TTL. An admin in org A who was merely a Door43 *member* of org B could switch to B and `PUT /api/admin/users/<self> {role:"admin"}` against B's database — the escalation then outlived the token. Fixed: the switch route re-resolves the role against the **target** workspace's DB and re-mints the access cookie (falling back to clearing it, forcing `/api/auth/refresh`).
  2. **Stale-tab cross-org writes.** `be_ws` is browser-wide, but each tab chose its outbox database and rendered its data at load time. A second tab left open in org A would then read org B's data and drain its queued edits onto org B's rows — silent corruption. Fixed: every request carries `X-Workspace`; the server 409s `workspace_mismatch`; the client reconciles and reloads; the outbox classifies that 409 as retryable rather than a version conflict so no op is dropped.
  3. **`workspaceEnv` was not re-entrant** — found only by driving it, not by review. It resolved bindings off the env it was handed; called on an already-swapped env (which is exactly what the new switch route does) any workspace bound to the literal name `"DB"` resolved to the *currently active* database. Symptom: switching back to the first workspace looked up roles in the wrong org and silently demoted the user to `viewer`. Fixed by always resolving bindings from a `BASE_ENV` reference to the original env; regression test added.
  4. **A brand-new org could never be onboarded.** `user_roles` is per-org, so the first person into a freshly created workspace landed as `viewer` and couldn't run the Setup wizard, and `SUPER_ADMINS` only granted the right to *switch*, not a role. Fixed: `SUPER_ADMINS` members now resolve to `admin` in every workspace via a new `effectiveRole()`; deliberately NOT folded into `lookupUserRole` because `adminUserRoutes`' last-admin guard asks about the `user_roles` row, not effective access.
  5. **Editing was completely broken in every workspace but the first** — the most severe finding, caught by the Codex review pass and then reproduced live. Moving `users`/`sessions` to `SHARED_DB` left the FKs behind: every per-workspace database still has `REFERENCES users(id)` on `tn_rows`/`tq_rows`/`twl_rows`/`verses`.`updated_by`, `edit_log.user_id`, `user_roles.added_by`, `pipeline_jobs.user_id`, `book_imports.imported_by`, `article_units.updated_by`, `l10n_overrides.updated_by` and more. The signed-in user's id comes from the shared DB, so in a second workspace it matches no local row and **D1 enforces foreign keys** (`PRAGMA foreign_keys` = 1, checked) — `POST /api/rows/tn` returned `D1_ERROR: FOREIGN KEY constraint failed` → 500. Fixed by *mirroring* the user row (same explicit `id`, never the access token) into the active workspace's `users` table via `ensureWorkspaceUser`, called from `attachAuth` behind an isolate-level memo and again on the switch route. Mirroring rather than dropping the FKs or repointing reads at `SHARED_DB`, because several display paths legitimately join `updated_by`/`edit_log.user_id` against the **local** `users` table to show who edited what.
  - **How verified:** repeated round-trip switches now hold roles correctly (admin in BSOJ ⇄ viewer in the test org, restored on return), the escalation attempt returns 403, `X-Workspace` mismatch returns 409 while an absent header passes, a super admin can onboard an empty workspace (`PUT /api/project-config` → 200), and the FK repro that returned 500 now returns 201 with `updated_by: 1` while the mirrored row carries a NULL `dcs_access_token`. Writes in the first workspace still work (regression-checked).
  - **Durable lesson:** *splitting tables across two databases does not split the foreign keys that point at them.* Before moving any table to `SHARED_DB`, grep `api/migrations/` for `REFERENCES <table>` — anything left behind in a per-workspace database will fail closed at write time, and only under multi-workspace configuration, so the single-org test path will never show it.
