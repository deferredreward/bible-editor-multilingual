...
- **Escalated (blocked on human):** the dev worker's `DCS_SERVICE_TOKEN` account (`BibleEditorService`, uid 53473) can write to existing BibleEditorMLTest repos but cannot CREATE repos there. Benjamin must either add BibleEditorService to an org team with "create repositories" permission (Owners works) or confirm the token has `write:organization` scope. After that, the next prefs save (or "Export context now") creates `BibleEditorMLTest/translation-context` automatically and the chain completes end-to-end. Also still unverified live (blocked on the same): stale-file deletion on DCS, and a translate pipeline run showing `contextRef` in the built options.
