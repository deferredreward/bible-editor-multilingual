...
- Three safety invariants, each with a test: manual rows are never overwritten/deleted by team sync (and editing a team row in the admin panel pins it to `'manual'`); the last remaining admin is never deleted by sync (same guard shape as `adminUserRoutes.ts`); a DCS failure returns "unknown" and skips the sync entirely, so an outage can't mass-revoke.
