...
2026-07-16 · **feat/per-user-work-mode (PR C, stacked on feat/manifest-inference / PR B #32)** — **Per-user work mode + Translate/Author toggle.**
- Migration `0052_user_work_mode.sql`: `users.work_mode TEXT CHECK IN ('translate','author')` + `dcs_orgs_json`/`dcs_orgs_fetched_at` (cached DCS org memberships — a seam for future per-user org switching, unused today). Applied to local dev D1.
- `api/src/auth.ts`: two paginated org-listing helpers — `fetchCurrentUserOrgs` (`/api/v1/user/orgs`, user token, OAuth path) and `fetchUserOrgsByLogin` (`/api/v1/users/{u}/orgs`, service token, refresh path) — both fail closed on ANY page failure (`OrgsFetchResult = {ok:true,orgs}|{ok:false}`; a partial page-1 result is discarded, never returned as "complete"). `isViewerOrgMember` refactored to sit atop them. OAuth callback stores the org cache best-effort: a fetch failure leaves `dcs_orgs_json`/`dcs_orgs_fetched_at` untouched (failure ≠ empty list); a successful empty list still stores `'[]'` + a fresh timestamp. `authMe` + `mintDevToken` both return `workMode` (parsed defensively, malformed/missing → `null`) and `orgs` (malformed JSON → `[]`; dev sign-in always `[]`). New `PUT /api/users/me/prefs` (`updateWorkModePrefs`, requireAuth + global CSRF, Zod `{workMode: enum(['translate','author']).nullable()}`).
- Web: `web/src/hooks/workModeCore.ts` — dependency-free pure module (`effectiveMode`, `applyOptimisticSet`, `reconcileFailure`) with its own test file (`workModeCore.test.mjs`, no React/api.ts/DOM). `web/src/hooks/useWorkMode.ts` wraps it in a module-level cache keyed by `userId` + a subscriber pattern (mirrors `useProjectConfig`): `seedWorkMode(userId, mode)` is called synchronously in `App.tsx`'s `useAuthGate` BEFORE `setState({kind:"ready",...})` on both the OAuth and dev sign-in paths, so the very first render already has the real stored mode instead of defaulting to Translate and firing a throwaway source request; `clearWorkMode()` fires on sign-out, on landing unauthenticated/denied, and is exposed for userId changes (no cross-account inheritance in one tab). `setWorkMode` is optimistic with monotonic-sequence stale-safe rollback (a late failure for a superseded request never clobbers a newer successful set) and surfaces `preferences.workMode.failed`.
- **Five `isTranslationProject` call sites re-keyed to effective mode** (`effectiveModeFor(workMode, cfg) === "translate"`): `ResourceColumn.tsx:355` (`translationMode`), `PipelineMenu.tsx:222` (translate-chapter options), `TopBar.tsx:211` (`showArticles`), `ArticleWorkspace.tsx:100` (`isTranslation`, direct-hash-nav guard), `PreferencesWorkspace.tsx:131`/`~205` (`memoryAvailable` + a new `authorMode` branch with its own copy). `Shell.tsx` intentionally untouched; `isTranslationProject` still exported meaning "project CAN translate" (a capability check, distinct from the per-user mode).
- **TN/TW quote-translation actions gated** (previously ungated by translationMode entirely): in `ResourceColumn.tsx`, the `onTranslateQuote`/`onStartQuoteBuild` props passed to both `WordsTable` (tW, 2 render paths) and `NoteCard` (tN) are now `translationMode ? handler : undefined`. AI Suggest and Add/Delete CRUD were left ungated per the affordance matrix (mode-independent). Demotion (`validated`→`edited` on content edit) is unconditional server-side in both modes — verified via a real node:sqlite regression test running rows.ts's literal CASE clause against `users.work_mode` set to each of `'translate'/'author'/null`.
- `WorkModeControl` (new, `PreferencesWorkspace.tsx`): MUI `ToggleButtonGroup` above `ProjectModeControl`, shown for non-viewers on translation-capable projects, "Saved for you" copy. `preferences.projectModeTitle/Intro` reworded to "Project settings (admin only)" / "…affects everyone in this project" to disambiguate from the new per-user control. Author-mode memory section shows `workMode.hiddenInAuthor` ("Hidden in Author mode — switch to Translate") — never the capability copy (`glOnly`), which is reserved for projects that can't translate at all.
- i18n: `preferences.workMode.{title,intro,translate,author,saved,failed,hiddenInAuthor}` + reworded `projectModeTitle`/`projectModeIntro`, added to all 14 locale files. **English text in all 14** (including the 13 non-English locales) — not hand-translated; same convention as PR A/B's `detectOrg`/`articles` keys. Tracked follow-up: a native-reviewed i18n pass for `workMode.*` + the reworded `projectMode*` keys across non-English locales.
- Tests: `api/src/auth.test.mjs` grew from 1 to ~15 blocks — org pagination (two-page concat, short-page termination, ANY page failure discards the whole result incl. network throw / non-JSON body), a `refreshToken` scenario proving a page-2 pagination failure fails viewer auth closed (not granted from the partial page-1 list), `updateWorkModePrefs` (enum reject, NULL round-trip, requires auth), `authMe`/`mintDevToken` workMode+orgs parity (malformed `dcs_orgs_json` → `[]`), a full `callbackDcsAuth` flow (via `startDcsAuth` to mint a real state cookie) proving org-fetch failure leaves the cache untouched while a successful empty list still stores `'[]'`+timestamp, and the node:sqlite demotion regression above. `web/src/hooks/workModeCore.test.mjs` covers `effectiveMode`'s byte-identical defaults and rollback sequencing incl. the stale-failure-does-not-clobber-newer-set case.
- Verified: `npm run typecheck` (api+web) clean; `npm run build` succeeds; api suite 90/90 (`node --experimental-strip-types --no-warnings --test "src/**/*.test.mjs"`); web suite 14/14 (`npm --workspace web run test`); migration applied to local dev D1 (`npx wrangler d1 migrations apply bible_editor_dev --local` — confirmed via `PRAGMA table_info(users)` that the three new columns exist). Did NOT run a browser smoke test per the calling agent's instructions.
- Note: found this worktree already had substantial partial implementation in place (auth.ts org-fetch/prefs plumbing, all five web re-key sites, `useWorkMode`/`workModeCore`, `WorkModeControl`, en.json keys) from an earlier interrupted session — completed the remainder (auth.test.mjs coverage, the 13 non-English locale files, quote-build gating alongside the already-gated translate-quote icon, this STATE.md entry) rather than redoing verified-good work.
- Code review (`/code-review --fix` + Codex gpt-5.5 → gpt-5.4-mini ×4, clean at round 5). Six fixes, all with tests: (1) sign-in fetched the org list twice → fetch once, reuse; (2) cross-session optimistic-rollback race → added a **session epoch** (`WorkModeCacheEntry.epoch`, bumped per seed) so a stale failed PUT can't roll back a different session/user — covers cross-user AND same-user quick re-login (seq resets to 0, epoch doesn't); (3) manifest inference auto-verified a standard lane repo whose manifest had a Bible subject but no `identifier` → require identifier present AND matching the role table; (4) both paginated org fetchers (`fetchPaginatedOrgNames`, `listOrgRepos`) returned `ok` with a truncated list at the 20-page cap → track reaching a short/404 final page and fail closed otherwise; (5) order-dependent candidate discovery with multiple `*_tn` repos → only derive langCode from a single tn repo; (6) `fetchPaginatedOrgNames` didn't treat a later-page 404 as end-of-list (Gitea does) → mirror `listOrgRepos`. Findings 3–5 touch PR B files (orgInference/orgRoutes) surfaced by the PR #33 review; they land via this stacked PR.
- Browser smoke PASSED: toggle renders (translation projects only), `PUT /api/users/me/prefs` persists + `/api/auth/me` carries `workMode`, Author mode hides the ResourceColumn source projection + Approve/Translate + TN/TW quote-translate + Articles entry + memory sections (with "switch to Translate" copy) while keeping Add/Delete + AI Suggest; no first-paint Translate flash; per-user isolation confirmed at the API layer; no console errors. One investigated non-issue: the scripture column's UHB/UGNT original-language line stays visible in Author mode — that's base text (the English root project shows it in author mode too), data-driven + version-toggle-controlled, NOT a translation-source panel, so correctly out of the affordance matrix's gating scope.
