...
- `node scripts/import-book.mjs <CODE>` then `(cd api && npx wrangler d1 execute bptranslate_dev --local --file=../scripts/out/import-<CODE>.sql)`
- `node scripts/import-lexicon.mjs` then `(cd api && npx wrangler d1 execute bptranslate_dev --local --file=../scripts/out/import-lexicon.sql)`
- `node scripts/refresh-verse.mjs <BOOK> <CH> <V> <VERSION>` then `(cd api && npx wrangler d1 execute bptranslate_dev --local --file=../scripts/out/refresh-<BOOK>-<CH>-<V>-<VERSION>.sql)`
...
- **ZEC 1:10 ULT** still has the pre-fix clumped-punctuation tail (the user observed `… earth , " { . " ` at the end of the verse). The serializer fix in `d434f206` is forward-only — re-saving the verse round-trips the existing bad shape. To recover:
  ...
  (cd api && npx wrangler d1 execute bptranslate_dev --local --file=../scripts/out/refresh-ZEC-1-10-ULT.sql)
  ```
