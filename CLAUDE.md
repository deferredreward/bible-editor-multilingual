...
- **Two admin UIs exist** — classic Preferences (`#/preferences`, `PreferencesWorkspace.tsx`) and the new admin desk (`#/admin/*` + `AdminDesk.tsx`'s "more tools" links) — with no shared nav registry, which is how sections have gone missing from one side before (issues #188/#189/#190). `web/src/adminSurfaceMap.ts` is the source of truth for which admin-facing features exist in each UI; its test (`adminSurfaceMap.test.mjs`) fails if a section/nav entry is added to either UI without a matching map entry. Adding or moving an admin feature in either UI: update the map in the same PR.
